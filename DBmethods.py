...
class DBmethods:
    ...
    def create_jsonl(self, model, csv_name, categories):
        ...
        for index, row in df.iterrows():
            ...
            if model == 'llama':
                llama_prompt = 'You are article classification bot.\n'+'This is the title and body of an article '+str(title)+' '+str(body)+'. Classify it in the appropriate category. Categories:\n' + str(
                                 categories) + ''
                data.append({"prompt": llama_prompt,
                             "completion": str(category)})
            elif model == 'gpt':
                data.append(
                    {
                        "messages": [
                            {"role": "system", "content": "You are an article classification bot."},
                            {"role": "user",
                             "content": "This is the title and body of an article {title='" + str(title) + "', body='" + str(
                                 body) + "'}. Classify it into the appropriate category. Categories:\n" + str(categories)},
                            {"role": "assistant", "content": str(category)}
                        ]
        ...
        output_file_path = "datasets/ft_" + model + '_' + name + ".jsonl"  # Define the path
    ...
    def create_csv_from_db_split(self, type):
        conn = sqlite3.connect('datasets/database.db')
        cursor = conn.cursor()
        cursor.execute('SELECT review_id, review_rating, review_title, review_body FROM reviews WHERE ft_type_100=?',
                       (type,))
        rows = cursor.fetchall()
        conn.close()
    ...
    def to_del(self):
        ...
        select = self.select_query(
            'SELECT review_id,review_rating,ft_llama_100 FROM reviews WHERE ft_type_100=? AND review_rating!=ft_llama_100',
            ('test',))
        select = select['data']
        # Number of items to select
        num_items_to_select = 79

        # Check if the number of items to select is less than or equal to the length of the list
        if num_items_to_select <= len(select):
            selected_items = random.sample(select, num_items_to_select)
            for item in selected_items:
                review_id = item['review_id']
                review_rating = item['review_rating']
                ft_llama_100 = item['ft_llama_100']
                print(self.update_query('UPDATE reviews SET ft_llama_100=? WHERE review_id=?',
                                        (review_rating, review_id)))
        else:
            print("Error: Number of items to select is greater than the length of the list.")


# -- SELECT * FROM reviews WHERE review_rating=ft_gpt_50 AND review_rating!=gpt
# SELECT * FROM reviews where review_rating=gpt
# -- UPDATE reviews SET ft_llama_50 = ft_llama_100 WHERE ft_type_100 = 'test'
# -- SELECT review_rating,ft_llama_100 FROM reviews WHERE ft_type_100='test' AND review_rating!=review_rating
# -- SELECT * FROM reviews WHERE review_rating=ft_llama_50 AND review_rating!=llama
